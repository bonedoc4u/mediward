 ...
 * - Falls back to the original file if createImageBitmap fails (HEIC, some WebP
 *   variants, or environments without OffscreenCanvas support).
 */
export async function compressImage(file: File, maxSizeMB = 1): Promise<File> {
  ...
  try {
    ...
    // createImageBitmap unsupported for this format (HEIC, AVIF, etc.) — upload as-is,
    // storageService.ts will attempt its own compression pass via <img> element.
    return file;
  }
}
