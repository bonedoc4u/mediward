...
const SuperAdminPanel: React.FC<{
  ...
  // ── Department grouping ────────────────────────────────────────────────────
  const [expandedDepts, setExpandedDepts] = useState<Set<string>>(new Set(['__pending__']));
  ...
  // Group hospitals: pending bucket first, then by department label
  const deptGroups = (() => {
    const pending: HospitalRow[] = hospitals.filter(h => h.status === 'pending');
    const rest = hospitals.filter(h => h.status !== 'pending');

    const byDept = new Map<string, HospitalRow[]>();
    for (const h of rest) {
      // Clean up department string: "DEPARTMENT OF ORTHOPAEDICS" → "Orthopaedics"
      const raw = h.department ?? '';
      const label = raw
        ? raw.replace(/^DEPARTMENT\s+OF\s+/i, '').replace(/\b\w/g, c => c.toUpperCase())
        : 'Uncategorised';
    ...
    const groups: { key: string; label: string; hospitals: HospitalRow[]; isPending?: boolean }[] = [];
    if (pending.length) groups.push({ key: '__pending__', label: 'Awaiting Approval', hospitals: pending, isPending: true });
    [...byDept.entries()]
      .sort(([a], [b]) => a.localeCompare(b))
      .forEach(([label, hs]) => groups.push({ key: label, label, hospitals: hs }));
    return groups;
  })();
          ...
          <div className="space-y-3">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-slate-800">Hospitals by Department</h2>
                <span className="text-xs text-slate-400 ml-1">{hospitals.length} total · {deptGroups.filter(g => !g.isPending).length} departments</span>
              </div>
            </div>

            {isLoadingHospitals ? (
              <div className="bg-white rounded-xl border border-slate-100 flex justify-center items-center py-16">
                <Loader2 className="w-6 h-6 animate-spin text-blue-500" />
              </div>
            ) : hospitals.length === 0 ? (
              <div className="bg-white rounded-xl border border-slate-100 text-center py-16 text-slate-400">
              ...
              deptGroups.map(group => {
                ...
                const activeCount    = group.hospitals.filter(h => h.status === 'active').length;
                const suspendedCount = group.hospitals.filter(h => h.status === 'suspended' || h.status === 'rejected').length;
                return (
                  <div key={group.key} className={`rounded-xl border overflow-hidden shadow-sm ${group.isPending ? 'border-amber-200' : 'border-slate-200'}`}>
                    ...
                    >
                      {isOpen
                        ? <ChevronDown className={`w-4 h-4 shrink-0 ${group.isPending ? 'text-amber-500' : 'text-slate-400'}`} />
                        : <ChevronRight className={`w-4 h-4 shrink-0 ${group.isPending ? 'text-amber-500' : 'text-slate-400'}`} />
                      }
                      {group.isPending
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`font-bold text-sm ${group.isPending ? 'text-amber-800' : 'text-slate-800'}`}>
                          {group.label}
                        </span>
                      </div>
                      {/* Pill counts */}
                      <div className="flex items-center gap-2 shrink-0">
                        {group.isPending ? (
                          <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-amber-500 text-white">
                            {group.hospitals.length} pending
                          </span>
                        ) : (
                          <>
                            {activeCount > 0 && (
                            ...
                            {suspendedCount > 0 && (
                              <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-slate-100 text-slate-500 border border-slate-200">
                                {suspendedCount} inactive
                              </span>
                            )}
                            <span className="text-xs text-slate-400">{group.hospitals.length} total</span>
                          </>
                        )}
                      </div>
                    </button>

                    {/* Hospital cards inside category */}
                    {isOpen && (
                          ...
                          return (
                            <div key={h.id} className="px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
                              ...
                              <div className="flex items-start gap-3 flex-1 min-w-0">
                                <div className={`shrink-0 w-9 h-9 rounded-lg flex items-center justify-center text-xs font-black ${
                                  h.status === 'active'    ? 'bg-green-100 text-green-700' :
                                  h.status === 'pending'   ? 'bg-amber-100 text-amber-700' :
                                  h.status === 'suspended' ? 'bg-slate-100 text-slate-500' :
                                                             'bg-red-100 text-red-600'
                                }`}>
                                  {h.name.slice(0, 2).toUpperCase()}
                                </div>
                                <div className="min-w-0 flex-1">
                                  <p className="font-semibold text-slate-800 text-sm leading-tight truncate">{h.name}</p>
                                  {h.slug && <p className="text-[11px] text-slate-400 truncate">/{h.slug}</p>}
                                  <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1">
                                    {h.adminName && (
                                      <span className="text-xs text-slate-500 flex items-center gap-1">
                                        <Users className="w-3 h-3" /> {h.adminName}
                                      </span>
                                    )}
                                    {h.adminEmail && (
                                      <span className="text-xs text-slate-400 truncate">{h.adminEmail}</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap shrink-0">
                                <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-slate-100 text-slate-600 uppercase">{h.plan}</span>
