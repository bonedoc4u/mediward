...
const SuperAdminPanel: React.FC<{
  ...
  // Sort: pending first, then active, then others; alphabetical within each
  const sortedHospitals = [...hospitals].sort((a, b) => {
    const order: Record<string, number> = { pending: 0, active: 1, suspended: 2, rejected: 3 };
    const diff = (order[a.status] ?? 9) - (order[b.status] ?? 9);
    return diff !== 0 ? diff : a.name.localeCompare(b.name);
  });

  // Helper: clean department label
  const deptLabel = (raw?: string) =>
    raw ? raw.replace(/^DEPARTMENT\s+OF\s+/i, '').replace(/\b\w/g, c => c.toUpperCase()) : null;
        ...
        {activeTab === 'hospitals' && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400 ml-1">{hospitals.length} total</span>
            </div>
            ...
            {isLoadingHospitals ? (
              ...
              <div className="divide-y divide-slate-100">
                {sortedHospitals.map(h => {
                  const isOpen   = expandedHospitals.has(h.id);
                  const isActing = actionId === h.id;
                  const dept     = deptLabel(h.department);
                  return (
                    <div key={h.id}>
                      {/* ── Hospital row (always visible) ── */}
                      <div className="flex items-center gap-3 px-5 py-4 hover:bg-slate-50/60 transition-colors">
                        {/* Expand toggle */}
                        <button
                          onClick={() => toggleHospital(h.id)}
                          className="shrink-0 p-1 rounded hover:bg-slate-200 transition-colors text-slate-400"
                          title={isOpen ? 'Collapse' : 'Show department details'}
                        >
                          {isOpen
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {h.slug && <p className="text-[11px] text-slate-400">/{h.slug}</p>}
                        </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {isActing ? (
                            ...
                            <>
                              {h.status === 'pending' && (
                                <>
                                  <button onClick={() => handleHospital(h.id, () => approveHospital(h.id), `"${h.name}" approved`)}
                                  ...
                                  <button onClick={() => handleHospital(h.id, () => rejectHospital(h.id), `"${h.name}" rejected`)}
                              ...
                              {(h.status === 'active' || h.status === 'suspended') && (
                                <>
                                  {h.status === 'active' && onViewWorkspace && (
                                    <button onClick={() => { onViewWorkspace(h.id, h.name); onSwitchToApp?.(); }}
                                  ...
                                  <button
                                    ...
                                    className={`flex items-center gap-1 px-3 py-1.5 text-xs font-bold rounded-lg transition-colors ${h.status === 'suspended' ? 'bg-green-100 hover:bg-green-200 text-green-700' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'}`}>
                                    {h.status === 'suspended'
                      ...
                      {/* ── Expanded: department details panel ── */}
                      {isOpen && (
                        <div className="mx-5 mb-4 rounded-xl border border-slate-200 bg-slate-50 overflow-hidden">
                          ...
                          <div className="flex items-center gap-2 px-4 py-3 bg-indigo-50 border-b border-indigo-100">
                            ...
                            <div>
                              <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-400">Registered Department</p>
                              <p className="text-sm font-bold text-indigo-900 leading-tight">
                                {dept ?? <span className="text-slate-400 font-normal italic">Not configured</span>}
                              </p>
                              {h.department && h.department !== dept && (
                                <p className="text-[10px] text-indigo-300 mt-0.5">{h.department}</p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-slate-200">
                            {[
                              { label: 'Admin',      value: h.adminName  ?? '—' },
                              { label: 'Email',      value: h.adminEmail ?? '—' },
                              { label: 'Plan',       value: h.plan.toUpperCase() },
                              { label: 'Registered', value: new Date(h.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) },
                            ].map(item => (
                              <div key={item.label} className="bg-white px-4 py-3">
                                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">{item.label}</p>
                                <p className="text-xs font-semibold text-slate-700 truncate" title={item.value}>{item.value}</p>
                              </div>
                            ))}
                          </div>
                          ...
                          <div className="flex items-center gap-2 px-4 py-3 border-t border-slate-200">
                            ...
                            {h.trialEndsAt && (
                              <span className="text-[11px] text-slate-500">
                                Trial ends: {new Date(h.trialEndsAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                              </span>
                            )}
                            {h.slug && (
                              <span className="text-[11px] text-slate-400 ml-auto">ID: {h.slug}</span>
                            )}
                          </div>
                        </div>
                      )}
